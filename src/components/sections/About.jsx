...
const About = () => {
  ...
  return (
    <section id="about" className="relative flex items-center min-h-[100svh] bg-transparent py-16 sm:py-24 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-10 animate-fade-in-up">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-5 rounded-2xl border border-[var(--color-border-subtle)]">
                <p className="font-display font-extrabold text-2xl text-[var(--color-text)] mb-1">2+</p>
                <p className="text-xs font-medium text-[var(--color-text-secondary)]">Años exp.</p>
              </div>
              <div className="bg-white p-5 rounded-2xl border border-[var(--color-border-subtle)]">
                <p className="font-display font-extrabold text-2xl text-[var(--color-text)] mb-1">25+</p>
                <p className="text-xs font-medium text-[var(--color-text-secondary)]">Proyectos</p>
              </div>
            </div>
          </div>

          {/* ── Columna Derecha: Texto y Biografía (7 columnas en Desktop) ── */}
          <div className="lg:col-span-7 lg:pl-6 flex flex-col text-center lg:text-left animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
            ...
            <div>
              ...
              <div className="flex flex-wrap justify-center lg:justify-start gap-3">
                {skills.map((skill) => (
                  ...
                  >
                    <span className="text-sm text-[var(--color-text-secondary)]">{skill.icon}</span>
                    {skill.name}
                  </span>
                ))}
              </div>
